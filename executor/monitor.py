...
#     try:
    output = runCmdRaiseException('qemu-img info -U -f %s --output json %s' % (disk_type, vdisk), use_read=True)
#     output = loads()
    print(output)
#     except:
#         output = {}
    if output:
        ...
        storage_vdisk_used_size_kilobytes.labels(zone, HOSTNAME, vdisk).set(vdisk_used_size)
...
def main():
    ...
    thread = threading.Thread(target=get_resource_collector_threads,args=())

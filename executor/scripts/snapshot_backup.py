...
class Domain(object):
    ...
    def backup_incremental(self, backup_dir):
    ...
    def merge_snapshot(self, bandwidth, base, timeout, async, keep_relative, in_bytes):
        """ Merges base to snapshot and removes old disk files """
        disks = self.get_disks()
        disk_files_tree = []
        for disk in disks:
            disk_files_tree += (DiskImageHelper.get_backing_files_tree(disk.file))
            merge_snapshot_cmd = "virsh blockpull --domain {domain_name} --path {disk_path} --wait".format(
    ...
    def backup_rotate_daily(self, backup_dir, rotate):
...
class DiskImageHelper(object):
    ...
    def get_backing_file(file):
        ...
        try:
            ...
            logging.warning("qemu-img info failed! Try with the argument -U")
            logging.debug("Executing: '%s'" % get_backing_file_cmd)
            out = subprocess.check_output(shlex.split(get_backing_file_cmd))
...
if __name__ == "__main__":
    ...
    backup_parser.add_argument('--action', action="store_const", const="backup", default="backup")
    backup_parser.add_argument('--domain', required=True)
    backup_parser.add_argument('--backup-dir', dest='backup_dir', required=True)
    merge_parser = subparsers.add_parser('merge')
    merge_parser.add_argument('--action', action="store_const", const="merge", default="merge")
    merge_parser.add_argument('--domain', required=True)
    merge_parser.add_argument('--bandwidth')
    merge_parser.add_argument('--base')
    merge_parser.add_argument('--timeout')
    merge_parser.add_argument('--async')
    merge_parser.add_argument('--keep-relative', dest='keep_relative')
    merge_parser.add_argument('--bytes', dest='in_bytes')
    merge_parser.add_argument('')
    rotate_parser = subparsers.add_parser('rotate')
    rotate_parser.add_argument('--action', action="store_const", const="backup", default="rotate")
    rotate_parser.add_argument('--rotate', default=1, type=int)
    rotate_parser.add_argument('--domain', required=True)
    rotate_parser.add_argument('--backup-dir', dest='backup_dir', required=True)
    parser.add_argument('--verbose', dest='verbose', default='INFO', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'],
                        type=str)
    ...
    try:
        ...
        if command_args.action == "backup":
            ...
            app_domain.merge_snapshot(command_args.bandwidth, command_args.base, command_args.timeout, \
                                      command_args.async, command_args.keep_relative, command_args.in_bytes)

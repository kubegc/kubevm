...
public class VirtualMachineNetworkWatcher extends AbstractWatcher implements Watcher<VirtualMachineNetwork> {
	...
	protected final static Logger m_logger = Logger.getLogger(VirtualMachineNetworkWatcher.class.getName());
	...
	public void eventReceived(Action action, VirtualMachineNetwork network) {
		...
		String podName = getPrefix() + "-" + network.getMetadata().getName() + "-" + namespace;
		
		if (action.toString().equals(ACTION_ADDED)) {
			Pod pod = null;;
			try {
				pod = createPod(network.getMetadata(), network.getSpec(), network.getSpec().getNodeSelector(),
						network.getSpec().getNodeName(), podName);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			if (client.pods().inNamespace(namespace).withName(podName).get() == null) {
				...
				m_logger.log(Level.INFO, "Create snapshot '" + network.getMetadata().getName() + "' in namespace '"
						+ network.getMetadata().getNamespace() + "'");
			...
			if (client.pods().inNamespace(namespace).withName(podName).get() != null) {
				...
				m_logger.log(Level.INFO, "Delete snapshot '" + network.getMetadata().getName() + "' in namespace '"
						+ network.getMetadata().getNamespace() + "'");
			}
		}
	}

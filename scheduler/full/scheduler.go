...
func (sched *Scheduler) scheduleOne() {
	...
	if err != nil {
        ...
        // It is not a Pod, but a customized Pod-like resource 
...
func SupportCRD(suggestedHost string, pod *v1.Pod, coreClient clientset.Interface, crdClient dynamic.Interface) {
	...
	if resource != nil {
		...
		resource.Object["metadata"].(map[string]interface{})["labels"] = labels
                ...
                // update CRD resource
		_, err := crdClient.Resource(gvr).Namespace(pod.GetAnnotations()["crdNamespace"]).Update(resource, metav1.UpdateOptions{})
		if err != nil {
			klog.Info(err)
		}

...
def run_virtctl():
    return runCmd('docker run -itd -h %s --net=host -v /etc/kubevmm:/etc/kubevmm -v /etc/sysconfig/cstor:/etc/sysconfig/cstor -v /dev:/dev -v /usr/lib64:/usr/lib64 -v /opt:/opt -v /var/log:/var/log -v /var/lib/libvirt:/var/lib/libvirt -v /var/run:/var/run -v /usr/bin:/usr/bin -v /usr/share:/usr/share -v /root/.kube:/root/.kube registry.cn-hangzhou.aliyuncs.com/cloudplus-lab/kubevirt-virtctl:%s bash virtctl.sh' % (HOSTNAME, VERSION))

def run_virtlet():
    return runCmd('docker run -itd -h %s --net=host -v /etc/kubevmm:/etc/kubevmm -v /etc/sysconfig/cstor:/etc/sysconfig/cstor -v /dev:/dev -v /usr/lib64:/usr/lib64 -v /opt:/opt -v /var/log:/var/log -v /var/lib/libvirt:/var/lib/libvirt -v /var/run:/var/run -v /usr/bin:/usr/bin -v /usr/share:/usr/share -v /root/.kube:/root/.kube registry.cn-hangzhou.aliyuncs.com/cloudplus-lab/kubevirt-virtlet:%s bash virtlet.sh' % (HOSTNAME, VERSION))
